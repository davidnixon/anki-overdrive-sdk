...
function decodeCarname(manufacturerData) {
   ...
   * - model_id: The model type of vehicle
  ...
  switch (vehicle_identifier) {
    ...
    case 0x1100efbe:
      ...
      break;
    case 0x1200efbe:
      carname = "International MXT";
      break;
    case 0x1300efbe:
      carname = "Ice Charger";
      break;
    case 0x1400efbe:
      carname = "Nuke Phantom";
      break;
    default:
      carname = `Unknown car 0x${vehicle_identifier.toString(16)}`;
  }
  debug(`Car name: ${carname}`);
  return carname;
}

function decodeDetails(manufacturerData) {
  /**
   * Vehicle hardware information encoded in the MANUFACTURER_DATA
   * record of an advertising packet.
   *
  ...
  // The above data description does not see quite right. I would expect model/product id to be the same for same cars.
  // For instance, for all Skulls, I would expect model_id to tbe the same, but they are not.
  // Or you might expect all cars to have the same model_id and all super trucks to have a different model_id. I do not find that either.
      // I think maybe the last 4 bytes might be a "Unique identifier for a physical vehicle". Let's decode it both ways.
  const vehicle_identifier = manufacturerData.readUInt32LE(0);
  const unique_id = manufacturerData.readUInt32LE(4);
  const model_id = manufacturerData.readUInt8(4);
  const reserved = manufacturerData.readUInt8(5);
  const product_id = manufacturerData.readUInt16LE(6);
  debug("manufacturerData", manufacturerData);
  debug(`vehicle_identifier: 0x${vehicle_identifier.toString(16)}`);
  debug(`model_id: 0x${model_id.toString(16)}`);
  debug(`reserved: 0x${reserved.toString(16)}`);
  debug(`product_id: 0x${product_id.toString(16)}`);
  debug(`unique_id: 0x${unique_id.toString(16)}`);
  return {vehicle_identifier, model_id, reserved, product_id, unique_id }
}
...
function decodeCarInformation(localName) {
  ...
  debug(coded);
  const state = coded.readUInt8(0);
  const VEHICLE_STATE_FULL_BATTERY = 1 << 4;
  const VEHICLE_STATE_LOW_BATTERY = 1 << 5;
  const VEHICLE_STATE_ON_CHARGER = 1 << 6;
  debug(`full battery: ${Boolean(state & VEHICLE_STATE_FULL_BATTERY)}`);
  debug(`low battery: ${Boolean(state & VEHICLE_STATE_LOW_BATTERY)}`);
  debug(`on charger: ${Boolean(state & VEHICLE_STATE_ON_CHARGER)}`);
  const firmware_version = coded.readUInt16LE(1);
