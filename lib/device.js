...
module.exports = class AnkiDevice {
  constructor(peripheral) {
    ...
    console.log(`Created device ${this.id} (${this.name})`);
  ...
  connect() {
    try {
      const that = this;
      console.log(`Connecting with ${that.id} (${that.name})`);
      const getService = function (services) {
        ...
        console.log(`Found services for ${that.id} (${that.name})`);
  ...
  activateSDKMode() {
    console.log(`Activating SDKMode for ${this.id} (${this.name})`);
    const that = this;
    if (!this.isConnected) {
  ...
  turnOnLogging() {
    console.log(`Turning logging on for ${this.id} (${this.name})`);
    let that = this;
    if (!that.isConnected) {
      return new Error("Car is not connected yet.");
    }
  ...
  stopAtLine() {
    ...
    this.events.on("localizationPositionUpdate", (message) => {
      switch (message.road_piece_id) {
        ...
        default:
          console.log(this.name, "road_piece_id", message.road_piece_id);
          break;
      }
    });
